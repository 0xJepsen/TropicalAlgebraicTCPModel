...
def simple_branch(conf):
    ...
    ps1 = PacketSink(3, env, rate=SWITCH_BANDWIDTH, qlimit=SWITCH_QSIZE)
    ps2 = PacketSink(4, env, rate=SWITCH_BANDWIDTH, qlimit=SWITCH_QSIZE)

    l1 = Link(env, LINK_BANDWIDTH)
    l2 = Link(env, LINK_BANDWIDTH)
    l3 = Link(env, LINK_BANDWIDTH)
    l4 = Link(env, LINK_BANDWIDTH)
    l5 = Link(env, LINK_BANDWIDTH)
