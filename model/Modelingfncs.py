            ...
            if i == 0:  # first router
                if j == 0:  # first packet
                    ...
                    init[j] = {
                        "departures": {
                            0: max(
                                init[j - init[j - 1]["V_n"]]["departures"][
                                    configuration.number_of_routers - 1
                                ]
                                + delay(
                                    configuration.number_of_routers - 1,
                                    0,
                                    configuration.link_rate,
                                    configuration.packet_to_size[j],
                                ),
                                init[j - 1]["departures"][0]
                                + sigma(
                                    configuration.switch_rate,
                                    configuration.packet_to_size[j],
                                ),
                            )
                        }
                ...
                init[j]["departures"][i] = the_max + sigma(
                    configuration.switch_rate, configuration.packet_to_size[j]
                )
...
def Z_init(packet_number, configuration):
    ...
    new = Matrix(
        dims=(configuration.number_of_routers * configuration.max_window, 1), fill=0
    )
...
def M_init(packet_number, configuration):
    ...
    """
    M = Matrix(
        dims=(configuration.number_of_routers, configuration.number_of_routers), fill=0
    )
        ...
        for j in range(configuration.number_of_routers):
            if i >= j:
                for _ in range(j, i + 1):
                    M[i, j] += sigma(
                        configuration.switch_rate,
                        configuration.packet_to_size[packet_number],
                    )
                for h in range(j, i):
                    M[i, j] += delay(
                        h,
...
def A_from_components(packet_number, configuration):
    ...
    k_epsilon = Matrix(
        dims=(configuration.number_of_routers, configuration.number_of_routers),
        fill=float("-inf"),
    )
    # print("KxK Epsilon: \n", k_epsilon)
